...
int main(int argc, char* argv[])
{
    ...
    while(!done)
    {
        ...
        switch(schedChoice)
        {
            ...
            case 2:
                ...
                if(procIdx == -1 || procList[procIdx].isDone){ //check if need to assign new process
                    procIdx = -1;
                    for(int i = 0; i < (int)procList.size(); i++){ //iterate through processes
                        
                        //if no valid process selected yet, select first one
                        if(procIdx == -1 && !procList[i].isDone && procList[i].startTime <= curTime)
                            procIdx = i;

                        //compare and find shortest process    
                        else if(!procList[i].isDone && procList[i].startTime <= curTime && procList[i].totalTimeNeeded < procList[procIdx].totalTimeNeeded)
                            procIdx = i;
                    }
                }
            ...
            case 3:
                ...
                for(int i = 0; i< (int)procList.size(); i++){ //iterate through processes
                    ...
                    //compare and find shortest remaining time
                    else if(procList[i].totalTimeNeeded - procList[i].timeScheduled < procList[procIdx].totalTimeNeeded - procList[procIdx].timeScheduled
                            && procList[i].startTime <= curTime && !procList[i].isDone)
                        procIdx = i;
                }
            ...
            case 4:
                ...
                if(procIdx == -1 || procList[procIdx].isDone){ //check if need to assign new process
                    ...
                    for(int i = 0; i< (int)procList.size(); i++){ //iterate through processes
                        ...
                        //compare and find high est ratio process
                        else if(!procList[i].isDone && procList[i].startTime <= curTime &&
                                (float)(procList[i].totalTimeNeeded + curTime - procList[i].startTime) / (float)procList[i].totalTimeNeeded >
                                (float)(procList[procIdx].totalTimeNeeded + curTime - procList[procIdx].startTime) / (float)procList[procIdx].totalTimeNeeded)
                            procIdx = i;
                    } 
                }
    ...
    //calculate individual and average turnaround times
    float total = 0;
    for(int i = 0; i < (int)procList.size(); i++){
        procList[i].turnaroundTime = procList[i].timeFinished - procList[i].startTime;

        procList[i].normalizedTurnaroundTime = procList[i].turnaroundTime / (float)procList[i].totalTimeNeeded;

        total += procList[i].normalizedTurnaroundTime;
    }

    float averageNormTurnTime = total / (float)procList.size();
